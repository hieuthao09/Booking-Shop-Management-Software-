    ...
    public partial class frmHoaDon : Form
    {
        ...
        private void btn_tim_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    tt += double.Parse(dgv_BaoCao[3, i].Value.ToString()) * double.Parse(dgv_BaoCao[2, i].Value.ToString());
                }
                lbl_Tongtien.Text = String.Format("{0: ##,##}", tt);
            }
            catch
            {
                MessageBox.Show("Không tìm được hóa đơn");
                return;
            }
        ...
        {
            InHoaDon frm = new InHoaDon(txt_mahd.Text);
            frm.ShowDialog();
        }
